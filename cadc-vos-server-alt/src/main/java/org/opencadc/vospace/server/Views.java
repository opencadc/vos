...
public class Views {
    ...
    private static void loadConfiguredViews() {
        ...
        try {
            ...
            for (String viewName : viewNames) {
                String viewAliasStr = rb.getString(viewName + "." + KEY_VIEW_ALIAS);
                String viewURIStr = rb.getString(viewName + "." + KEY_VIEW_URI);
                ...
                URI viewURI;
                try {
                    viewURI = new URI(viewURIStr);
                } catch (URISyntaxException e) {
                    throw new ExceptionInInitializerError("URI reference for "
                            + configClass.getName() + " has an invalid syntax: " + viewURIStr);
                }

                URI viewAlias;
                try {
                    viewAlias = new URI(viewAliasStr);
                } catch (URISyntaxException e) {
                    throw new ExceptionInInitializerError("Alias reference for "
                            + configClass.getName() + " has an invalid syntax: " + viewAliasStr);
                }
                ...
                try {
                    ...
                    if (acceptsValue.trim().equalsIgnoreCase("true")) {
                        accepts.add(viewURI);
                    }
                } catch (MissingResourceException e) {
                ...
                try {
                    ...
                    if (providesValue.trim().equalsIgnoreCase("true")) {
    ...
    */
    ...
    public List<AbstractView> getViews() throws Exception {
        Collection<URI> viewURIs = uriViewMap.keySet();
        Class<AbstractView> nextViewClass;
        ...
        for (URI viewURI : viewURIs) {
            ...
            nextView = createAbstractView(nextViewClass, viewURI);

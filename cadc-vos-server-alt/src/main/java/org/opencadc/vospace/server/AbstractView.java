 ...
 * of a View.
 *
 * <p>Generally, subclasses should implement setNode(Node, String) and either:
 *
 * <p>1) For views that result in a redirect:
 * - getRedirectURL()
 *
 * <p>or, the set of:
 *
 * <p>2) For views that result in data returned:
 * - write(OutputStream)
 * - getContentLength()
 * - getMediaType()
 * - getEncodings()
 * - getContentMD5()
 *
 * <p>The VOSpaceAuthorizer and NodePersistence objects are available to view implementations.
 *
 * <p>Please note:  The node returned by getNode() has already gone through an authorization
...
public abstract class AbstractView extends View {
     ...
     *
     * <p>Note: The node available in getNode() has already gone
    ...
    public Date getLastModified() {
        NodeProperty modificationDate = node.getProperty(VOS.PROPERTY_URI_DATE);
        if (modificationDate != null) {
            try {
                DateFormat df = DateUtil.getDateFormat(DateUtil.IVOA_DATE_FORMAT, DateUtil.UTC);
                return df.parse(modificationDate.getValue());
            } catch (ParseException e) {
                log.warn("Date " + modificationDate.getValue()
                        + " could not be parsed.");
            }
        }
        return null;
